...
def calculate_score(row):
    ...
    elif not pd.isna(eps) and eps < 0:
        score -= 2
    ...
    elif not pd.isna(net_profit) and net_profit < 0:
        score -= 2
    ...
    if not pd.isna(ps):
        if ps <= GOOD_PS:
            score += 2
            reasons.append("attractive P/S")
        elif ps >= BAD_PS:
            score -= 2
            reasons.append("expensive P/S")
    ...
    if not pd.isna(gross_margin):
        if gross_margin >= 50:
            ...
            score -= 1
            reasons.append("weak gross margin")
    ...
    return recommendation, score, ", ".join(reasons)
    ...
    for idx, ticker in enumerate(TICKERS):
        try:
            ...
            row = {
                ...
                "USD Revenue": revenue_usd,
                "USD Net Profit": net_profit_usd,
                "Revenue Growth %": revenue_growth,
                "Gross Margin %": gross_margin,
                "Free Cash Flow": fcf_usd,
                "P/S": ps_ratio,
                "P/E": pe_ratio,
                "EPS": eps
            }

            recommendation, score, reason = calculate_score(row)
...
def format_table(df):
    return (
        ...
        .format({
            "Price": "${:.2f}",
            "Today %": "{:+.2f}%",
            "Market Cap": money_fmt,
            "USD Revenue": money_fmt,
            "USD Net Profit": money_fmt,
            "Revenue Growth %": "{:+.2f}%",
            "Gross Margin %": "{:.2f}%",
            "Free Cash Flow": money_fmt,
            "P/S": "{:.2f}",
            "P/E": "{:.2f}",
            "EPS": "{:.2f}",
            "Score": "{:.0f}"
...
st.title("Stock Fundamentals + Recommendation Dashboard")

st.caption(
    "Rule-based dashboard using Yahoo Finance data. Revenue, net profit, and free cash flow are converted to USD when Yahoo reports a non-USD financial currency."
)

if st.button("Refresh Yahoo Finance Data"):
    st.cache_data.clear()
...
col1.metric("Strong Buy", len(strong_buy_df))
col2.metric("Buy", len(buy_df))
...
This dashboard uses a simple transparent score:

- Positive EPS: +2
- Positive net profit: +2
- P/S ≤ 5: +2
- P/E between 0 and 25: +2
- Revenue growth ≥ 25%: +2
- Revenue growth 10% to 25%: +1
- Gross margin ≥ 50%: +1
...
st.download_button(
    "Download Full CSV",
    csv,
    "stock_recommendation_dashboard.csv",
    "text/csv"
)